...
export class SyncService {
    ...
    public async syncProductToEs(productId: string): Promise<void> {
        ...
        try {
            ...
            if (esDocument) {
                ...
                this.logger.warn(`Product ${productId} not found or invalid. Skipping.`);
    ...
    try {
      ...
      if (!spuData || spuData.variants.length === 0) {
        this.logger.warn(`SPU ${spuId} not found or has no variants. Skipping.`);
        return;
      }
      ...
      this.logger.log(`Successfully synced ${skuDocuments.length} SKUs for SPU ${spuId}.`);
